...
function drawLabelValue(doc, label, valor, marginL) {
  doc
    ...
    .text(`${label}:`, marginL, doc.y, { continued: true, width: 200 })
    .font('Helvetica')
    .fillColor('#222222')
    .text(`  ${valor ?? '—'}`);
  doc.moveDown(0.2);
...
function buildBusinessPDF(data) {
  return new Promise((resolvePromise, reject) => {
    ...
    drawLabelValue(doc, 'Dirección',           data.direccionCliente,   marginL);
    ...
    doc
      ...
      .fontSize(10)
      .fillColor(COLOR_PRIMARY)
      .text('DETALLE DEL SERVICIO', marginL, doc.y);

    doc.moveDown(0.4);

    drawLabelValue(doc, 'Tipo de Operación',   data.tipoOperacion,       marginL);
    drawLabelValue(doc, 'Descripción',         data.descripcionServicio, marginL);
    ...
    drawLabelValue(doc, 'Red Utilizada',    'Stellar Network', marginL);

    doc.moveDown(0.3);

    // TXID completo en monoespaciado
    doc
      .font('Helvetica-Bold')
      .fontSize(9)
      .fillColor(COLOR_GRAY)
    ...
    drawTableRow(doc, 'CONCEPTO', 'VALOR', {
      bold: true, bgColor: COLOR_PRIMARY, leftX: tableX,
    });

    const fuenteTC = data.fuenteTipoCambio
      ? ` (Fuente: ${data.fuenteTipoCambio})`
      : '';
    ...
    const filas = [
      ...
      ['Spread FX',                                 data.spreadFx != null ? `- ${formatBOB(data.spreadFx)}` : 'N/A'],
    ];
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // SECCIÓN 6 — INFORMACIÓN DE BANCARIZACIÓN (RND 102400000021)
    // ═══════════════════════════════════════════════════════════════════════
    ...
    doc
      ...
      .fontSize(10)
      .fillColor(COLOR_PRIMARY)
      .text('INFORMACIÓN DE BANCARIZACIÓN', marginL, doc.y);
    ...
    if (hasBancData) {
      ...
      drawLabelValue(doc, 'Titular Cuenta Origen',   b.payerAccountHolder,    marginL);
      ...
      drawLabelValue(doc, 'Referencia Doc. de Pago',  b.paymentDocReference,   marginL);
    } else {
      const reqNote = data.montoOrigenBOB >= 50000
        ? 'NOTA: Transacción ≥ Bs 50.000 — respaldo de bancarización obligatorio (RND 102400000021). Datos pendientes de registro.'
        : 'Transacción bajo Bs 50.000 — respaldo de bancarización no exigido por normativa.';
      doc
        .font('Helvetica')
        .fontSize(8.5)
        .fillColor(COLOR_GRAY)
    ...
    doc
      ...
      .fillColor(COLOR_GRAY)
      .text(
        `Comprobante N° ${data.invoiceNumber}  ·  `
    ...
    if (data.qrDataUrl) {
      ...
      doc.image(data.qrDataUrl, qrX, qrY, { width: qrSize, height: qrSize });

      doc
        .font('Helvetica')
        .fontSize(7)
        .fillColor(COLOR_GRAY)
        .text('Escanear para verificar', qrX - 10, qrY + qrSize + 4, {
          width: qrSize + 20,
          align: 'center',
        });
    }
